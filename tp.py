...
def useradd(usr):
    ...
    if user == 'core' or cudo_:
        ...
        if cur.fetchone():
            ...
            cur.execute("UPDATE inode SET perm='{}' WHERE ind={}".format('750', int(ind)))
...
def cp(c):
    ...
    if user == 'core' or user == findOwner(d):
        ...
        if p[6] == 'r' and p[8] == 'x':
            perm = True
    if not perm:
        print("E: " + d.split('\\')[-1].split('-')[0] + ": You dont have permission")
        return
    #breakpoint()
    perm = False
    for i in os.listdir():
        if i.endswith(c[1]):
            ...
            if user == 'core' or user == findOwner(i):
                ...
                if p[6] == 'r':
                    ...
                    break
    if not perm:
        print("E: " + c[1] + ": you dont have permission")
        return
    if c[2].startswith('/'):
        if c[2] == '/':
            perm = False
            ...
            if user == 'core' or user == findOwner('0-ss'):
                ...
                if p[7] == 'w' and p[8] == 'x':
                    perm = True
            if not perm:
                print("E: / : you dont have permission")
                return
                    ...
                    if j.endswith(j) and os.path.isdir(j):
                        ...
                        if user == 'core' or user == findOwner(j):
                            ...
                            if p[1] == 'w' and p[2] == 'x':
                                ...
                                print("E: " + i + ": you dont have permission")
                                return
                        elif isGroup(j):
                            found = True
                            if p[4] == 'w' and p[5] == 'x':
        ...
        if i.endswith(f) and os.path.isfile(i):
            ...
            if user == 'core' or user == findOwner(i):
                if p[2] == 'x':
                    ...
                    print("E: " + f + ": you dont have permission")
                    return False
            elif isGroup(i):
                if p[5] == 'x':
                    return True
...
if first_try:
    ...
    cur.execute("INSERT INTO inode VALUES(3, '{}', '{}', '{}', '750')".format(un, un, un))
    cur.execute("INSERT INTO inode VALUES(4, 'boot', 'core', 'core', '755')")
    os.mkdir('C:/Users/sanjsark/SarkSys/0-ss/4-boot')
    cur.execute("INSERT INTO inode VALUES(5, 'etc', 'core', 'core', '755')")
    os.mkdir('C:/Users/sanjsark/SarkSys/0-ss/5-etc')
